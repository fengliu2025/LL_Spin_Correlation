...
#ifndef ntp_Lambda_ME_h
#define ntp_Lambda_ME_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   ntp_Lambda_ME(std::vector<std::string> Infiles, std::string Outfile);
   virtual ~ntp_Lambda_ME();
   ...
   virtual void     Rotation(TVector3 a_hat,TVector3 b_hat, TVector c_hat, TLorentzVector *v);
};

#endif

#ifdef ntp_Lambda_ME_cxx
ntp_Lambda_ME::ntp_Lambda_ME(std::vector<std::string> Infiles, std::string Outfile,ntp_Lambda_Reader *Reader) 
   : fChain(0), InputFiles(Infiles), OutputFile(Outfile), MixEvent_Reader(Reader)
{

}

ntp_Lambda_ME::~ntp_Lambda_ME()
{
   
}

Int_t ntp_Lambda_ME::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ntp_Lambda_ME::LoadTree(Long64_t entry)
{
...
void ntp_Lambda_ME::Init(TTree *tree)
{
...
void ntp_Lambda_ME::Show(Long64_t entry)
{
...
Int_t ntp_Lambda_ME::Cut(Long64_t entry)
{
